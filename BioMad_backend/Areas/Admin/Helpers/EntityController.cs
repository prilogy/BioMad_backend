...
{
    public abstract class EntityController<T, T2> : AdminController
        ...
        where T2 : Translation<T2>, ITranslationEntity<T>, IWithName, new()
    {
        ...
        public async Task<IActionResult> Edit(int? id, string translationAction, int translationId, int cultureId)
        {
            if (id == null)
                return NotFound();
